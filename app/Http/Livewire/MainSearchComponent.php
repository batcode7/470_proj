...
class MainSearchComponent extends Component
{
    ...
    public function render()
    {
        ...
        {
            $products = Product::where('name','like','%'.$this->search .'%')->where ('catagory_id','like','%'.$this -> product_catagory_id.'%')->orderBy('regular_price','ASC')->paginate($this -> pagesize);

        }
        else if($this->sorting == "price-desc")
        {
            $products = Product::where('name','like','%'.$this->search .'%')->where ('catagory_id','like','%'.$this -> product_catagory_id.'%')->orderBy('regular_price','DESC')->paginate($this -> pagesize);

        }else 
        {
            $products = Product::where('name','like','%'.$this->search .'%')->where ('catagory_id','like','%'.$this -> product_catagory_id.'%')->paginate($this -> pagesize);
        }
        
        $catagories = Catagory::all();

        
        return view('livewire.main-search-component',['products'=> $products,'catagories'=>$catagories])->layout('layouts.base');
    }
}